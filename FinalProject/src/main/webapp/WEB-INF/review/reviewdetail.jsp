...
<body>
	...
	<c:set value="${empname }" var="empname"/>
		...
		<div class="total">
		<!-- 기업 총 평가 평균 -->
		<b style="font-size: 5em;">${avgRating }</b>&nbsp;&nbsp;&nbsp;
		<span class="star-rating">
		<!-- 너비 100%에 맞추려면 (rating*10)*2 -->
		<span style ="width:${(avgRating*10)*2}%"></span>
		</span>
		<!-- 기업 세부 평가점수 차트 -->
		...
		<table class="table table-bordered" style="width: 700px; margin-left: 100px;">
			<tr>
				<td colspan="2">
					<h4 style="color: gray;">${data.job_group} | ${data.prenow} |
					<fmt:formatDate value="${data.writeday }" pattern="MM-dd" var="date"/>${date}
					</h4>
				</td>
			</tr>
			<tr>
