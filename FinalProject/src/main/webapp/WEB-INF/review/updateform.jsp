...
<body>
	...
	<div class="myreview">
		...
		<c:forEach var="data" items="${updatedata }" varStatus="n">
    		...
    		<h3 style="color: #021B80; margin-top: 100px; margin-bottom: 30px;">${emp } 기업리뷰 수정</h3>    	
    		<tr>
    			<th>
    				    <input type="hidden" name="empname" value="${emp }"
    					style="width: 300px; height: 40px;" required="required" class="form-control">기업명 <b style="font-weight: bold;">${data.empname }</b>
    			</th>
    		</tr>
    		<tr>
    			<th>
    				<h5 >현 직장/전 직장 <b style="font-weight: bold;">${data.prenow }</b></h5>
    			</th>
    		</tr>
    		<tr>
    			<th>
    				<h5>직종 <b style="font-weight: bold;">${data.job_group }</b></h5></h5><br>
    			</th>
    		</tr>
    		<tr>
    			<th>
    	...
    	<br>
    	<button type="submit" class="btnsubmit btn btn-default" style="margin-left: 400px;" >수정</button>
    	<button type="button" class="btnsubmit btn btn-default" onclick="history.back()" >취소</button>
    	<br><br><br>
    </form>
    </c:forEach>
	</div>
